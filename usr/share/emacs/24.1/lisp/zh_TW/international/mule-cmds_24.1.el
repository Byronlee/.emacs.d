...
;;; 2013-05-19 22:08:07 CST translated
...
(defvar describe-language-environment-map
  (let ((map (make-sparse-keymap "語言環境說明")))
    (define-key map
      [Default] `(menu-item ,(purecopy "默認值") describe-specified-language-support))
    map))

(defvar setup-language-environment-map
  (let ((map (make-sparse-keymap "設置語言環境")))
    (define-key map
      [Default] `(menu-item ,(purecopy "默認值") setup-specified-language-environment))
    map))

(defvar set-coding-system-map
  (let ((map (make-sparse-keymap "設置文字編碼")))
    (define-key-after map [universal-coding-system-argument]
      `(menu-item ,(purecopy "用于下一次命令") universal-coding-system-argument
        :help ,(purecopy "Coding system to be used by next command")))
    (define-key-after map [separator-1] menu-bar-separator)
    (define-key-after map [set-buffer-file-coding-system]
      `(menu-item ,(purecopy "用于保存當前緩沖區") set-buffer-file-coding-system
        :help ,(purecopy "How to encode this buffer when saved")))
    (define-key-after map [revert-buffer-with-coding-system]
      `(menu-item ,(purecopy "用于立即重新加載當前文件")
    ...
    (define-key-after map [set-selection-coding-system]
      `(menu-item ,(purecopy "用于 X 選擇/剪貼板") set-selection-coding-system
    ...
    (define-key-after map [set-next-selection-coding-system]
      `(menu-item ,(purecopy "用于下一次 X 選擇") set-next-selection-coding-system
        :visible (display-selections-p)
        :help ,(purecopy "How to en/decode next selection/clipboard operation")))
    (define-key-after map [set-buffer-process-coding-system]
      `(menu-item ,(purecopy "用于 I/O 子進程") set-buffer-process-coding-system
...
(defvar mule-menu-keymap
  (let ((map (make-sparse-keymap "Mule(多國語言環境)")))
    (define-key-after map [set-language-environment]
      `(menu-item  ,(purecopy "設置語言環境") ,setup-language-environment-map))
    (define-key-after map [separator-mule] menu-bar-separator)

    (define-key-after map [toggle-input-method]
      `(menu-item ,(purecopy "切換輸入法") toggle-input-method))
    (define-key-after map [set-input-method]
      `(menu-item ,(purecopy "選擇輸入法...") set-input-method))
    ...
    (define-key-after map [set-various-coding-system]
      `(menu-item ,(purecopy "設置文字編碼") ,set-coding-system-map
		  :enable (default-value 'enable-multibyte-characters)))
    (define-key-after map [view-hello-file]
      `(menu-item ,(purecopy "顯示多國語言文本") view-hello-file
    ...
    (define-key-after map [list-character-sets]
      `(menu-item ,(purecopy "列出字符集") list-character-sets
        :help ,(purecopy "Show table of available character sets")))
    (define-key-after map [mule-diag]
      `(menu-item ,(purecopy "顯示全部 Mule 狀態") mule-diag
  ...
  (interactive
   (let ((default (and buffer-file-coding-system
     ...
     (list (read-coding-system
	    (if default
		(format "接下來的命令使用的文字編碼: (默認為 %s)" default)
	      "接下來的命令使用的文字編碼: ")
	    default))))
  (let* ((keyseq (read-key-sequence
		  (format "用 %s 來執行的命令" coding-system)))
...
character found, or nil if all characters are encodable."
  ...
  (let ((pos (unencodable-char-position (point) (point-max) coding-system)))
    (if pos
	(goto-char (1+ pos))
      (message "後面的字符可以按 %s 進行編碼" coding-system))
    pos))
...
e.g., for sending an email message.\n ")
		...
		(princ " ") (princ x))
	      (insert "\n"))
	    (when unsafe
	      (insert (if rejected "其它文字編碼"
			"但是，當中的每一種")
		      " 有沖突的字符無法進行編碼:\n")
	      (dolist (coding unsafe)
		(insert (format "  %s 無法對這些進行編碼:" (car coding)))
		(let ((i 0)
		      (func1
		       #'(lambda (bufname pos)
  ...
  (let ((describe-map describe-language-environment-map)
	...
	(let ((l parents)
	  ...
	  (while l
	    ...
	    (or map (setq prompt (format "%s 環境" parent)))
	    (if (not map)
		(progn
...
(defgroup leim nil
  "LEIM: 即 Emacs 輸入法庫 (Libraries of Emacs Input Methods)."
  :group 'mule)
...
instead of the one selected last time.  It is a symbol or a string."
  (setq input-method
	(or input-method
	    current-input-method
	    default-input-method
	    (read-input-method-name "輸入法: " nil t)))
  (if (and input-method (symbolp input-method))
      (setq input-method (symbol-name input-method)))
...
See also the variable `input-method-highlight-flag'."
  :type '(choice (const :tag "總是" t) (const :tag "從不" nil)
  ...
  (let ((doc (get-language-info language-name 'documentation)))
	...
	(let ((input-method (get-language-info language-name 'input-method))
	  ...
	  (when (and input-method
	    ...
	    (insert "輸入法 (default " (car input-method) ")\n")
...
properties are sticky."
  (interactive
   (list (read-char-by-name "Unicode (名稱或16進制)")
  ...
  (cond
   ((null character)
    (error "不是一个 Unicode 字符"))
   ((not (integerp character))
    (error "不是一个 Unicode 字符代碼: %S" character))
   ((or (< character 0) (> character #x10FFFF))
    (error "不是一个 Unicode 字符代碼: 0x%X" character)))
  (if inherit
