...
(defun solar-setup ()
  ...
  (or calendar-longitude
      (setq calendar-longitude
            (solar-get-number
             "输入10进制经度(longitude); + 代表东(E), - 代表西(W): ")))
  (or calendar-latitude
      (setq calendar-latitude
            (solar-get-number
             "输入10进制纬度(longitude); + 代表北(N), - 代表南(S): ")))
...
Optional NOLOCATION non-nil means do not print the location."
  (let ((l (solar-sunrise-sunset date)))
    (format
     "%s, %s%s(白天有%s小时)"
     (if (car l)
         (concat "日出 " (apply 'solar-time-string (car l)))
       "没有日出")
     (if (cadr l)
         (concat "日落 " (apply 'solar-time-string (cadr l)))
       "没有日落")
     (if nolocation ""
       (format " 在 %s" (eval calendar-location-name)))
     (nth 2 l))))
