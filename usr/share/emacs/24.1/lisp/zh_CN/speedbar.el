...
(defvar speedbar-easymenu-definition-base
  (append
   '("快捷栏"
     ["更新" speedbar-refresh t]
     ["自动更新" speedbar-toggle-updates
   ...
   (if (and (or (fboundp 'defimage)
		...
		(display-graphic-p)
	      window-system))
       (list
	["使用图像" speedbar-toggle-images
	 :style toggle :selected speedbar-use-images]))
   )
  "Base part of the speedbar menu.")

(defvar speedbar-easymenu-definition-special
  '(["在线编辑条目" speedbar-edit-line t]
    ["显示全部文件" speedbar-toggle-show-all-files
     :style toggle :selected speedbar-show-unknown-files]
    ["展开文件 Tags 标识" speedbar-expand-line
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *.\\+. "))]
    ["大量缓存和展开" speedbar-flush-expand-line
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *.\\+. "))]
    ["展开全部的折叠项" speedbar-expand-line-descendants
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *.\\+. ")) ]
    ["折叠文件 Tags 标识" speedbar-contract-line
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *.-. "))]
;    ["Sort Tags" speedbar-toggle-sorting
;     :style toggle :selected speedbar-sort-tags]
    "----"
    ["文件/Tag 标识信息" speedbar-item-info t]
    ["加载 Lisp 文件" speedbar-item-load
     (save-excursion
       (beginning-of-line)
       (looking-at "[0-9]+: *\\[[+-]\\] .+\\(\\.el\\)\\( \\|$\\)"))]
    ["字节编译文件" speedbar-item-byte-compile
     (save-excursion
       (beginning-of-line)
       (looking-at "[0-9]+: *\\[[+-]\\] .+\\(\\.el\\)\\( \\|$\\)"))]
    ["复制文件" speedbar-item-copy
     (save-excursion (beginning-of-line) (looking-at "[0-9]+: *\\["))]
    ["重命名文件" speedbar-item-rename
     (save-excursion (beginning-of-line) (looking-at "[0-9]+: *[[<]"))]
    ["创建目录" speedbar-create-directory
     (save-excursion (beginning-of-line) (looking-at "[0-9]+: *[[<]"))]
    ["删除文件" speedbar-item-delete
     (save-excursion (beginning-of-line) (looking-at "[0-9]+: *[[<]"))]
    ["删除对象" speedbar-item-object-delete
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *\\[[+-]\\] [^ \n]+ \\*?[!#]$"))]
    )
...
(defvar speedbar-easymenu-definition-trailer
  (append
   (if (and (featurep 'custom) (fboundp 'custom-declare-variable))
       (list ["自定义..." speedbar-customize t]))
   (list
    ["关闭" dframe-close-frame t]
    ["退出" delete-frame t] ))
...
`speedbar-before-delete-hook' is called before the frame is deleted."
  ...
  (if (not (buffer-live-p speedbar-buffer))
      (save-excursion
	(setq speedbar-buffer (get-buffer-create " 快捷栏"))
...
(define-derived-mode speedbar-mode fundamental-mode "快捷栏"
...
\\{speedbar-key-map}"
  (save-excursion
    ...
    (setq frame-title-format (concat "快捷栏 " speedbar-version)
...
File style information is displayed with `speedbar-item-info'."
  (save-excursion
    ...
    ;; Get the text
    (speedbar-message "文本: %s" (buffer-substring-no-properties
		  ...
		  (with-no-warnings
			...
			(speedbar-message
			 (funcall semantic-sb-info-format-tag-function detail parent))
		      (if parent
			  (speedbar-message "详情: %s of tag %s" detail
					    (if (semantic-tag-p parent)
						(semantic-format-tag-name parent nil t)
					      parent))
			(speedbar-message "详情: %s" detail))))
		;; Not using `semantic':
		(if parent
		    (speedbar-message "详情: %s of tag %s" detail parent)
		  (speedbar-message "详情: %s" detail))))
	  nil)))))
...
Files can be copied to new names or places."
  ...
  (let ((f (speedbar-line-file)))
	...
	(error "Cannot copy directory")
      (let* ((rt (read-file-name (format "复制 %s 到: "
	...
	(if (or (not (file-exists-p rt))
		(speedbar-y-or-n-p (format "覆盖 %s (用 %s)? " rt f)
				   t))
	    (progn
	      (copy-file f rt t t)
...
Files can be renamed to new names or moved to new directories."
  ...
  (let ((f (speedbar-line-file)))
    (if f
	(let* ((rt (read-file-name (format "重命名 %s 为: "
					   (file-name-nondirectory f))
				   (file-name-directory f)))
	       (refresh (member (expand-file-name (file-name-directory rt))
				speedbar-shown-directories)))
...
(defun speedbar-item-delete ()
  ...
  (let ((f (speedbar-line-file)))
    (if (not f) (error "不是文件"))
    (if (speedbar-y-or-n-p (format "删除 %s? " f) t)
	(progn
	  (if (file-directory-p f)
	      (delete-directory f t t)
	    (delete-file f t))
	  (speedbar-message "确认执行.")
	  (let ((p (point)))
	    (speedbar-refresh)
	    (goto-char p))
	  ))
...
variable `speedbar-obj-alist'."
  ...
  (let* ((f (speedbar-line-file))
    ...
    (if (and oa (file-exists-p obj)
	     (speedbar-y-or-n-p (format "删除 %s? " obj) t))
	(progn
	  (delete-file obj)
	  (speedbar-reset-scanners)))))
...
(defun speedbar-change-initial-expansion-list (new-default)
  ...
  (interactive
   (list
    (completing-read (format "快捷栏模式(默认为 %s): "
			     speedbar-previously-used-expansion-list-name)
		     speedbar-initial-expansion-mode-alist
		     nil t "" nil
		     speedbar-previously-used-expansion-list-name)))
...
Argument LST is the list of tags to sort into groups."
  (let ((newlst nil)
    ...
    (if (not newlst)
	(nreverse sublst)
      (setq newlst (cons (cons "Tags标识" (nreverse sublst)) newlst))
      (nreverse newlst))))
...
(defun speedbar-update-localized-contents ()
  ...
  (let ((name speedbar-initial-expansion-list-name)
	...
	(if (or (member (expand-file-name default-directory)
	    ...
	    nil
	  (if (<= 1 speedbar-verbosity-level)
	      (speedbar-message "更新快捷栏到: %s..."
				default-directory))
	  (speedbar-update-directory-contents)
	  (if (<= 1 speedbar-verbosity-level)
	      (progn
		(speedbar-message "更新快捷栏到: %s...完成"
				  default-directory)
...
the file being checked."
  (let* ((d (string-to-number depth))
	 ...
	 (fulln (concat f fn)))
    (if (<= 2 speedbar-verbosity-level)
	(speedbar-message "快捷栏版本控制检查...%s" fulln))
...
(defun speedbar-directory-line (directory)
  ...
  (save-match-data
    ...
    (let ((nomatch t) (depth 0)
	  ...
	  (error "内部错误: 文件 %s 在快捷栏中未显示" directory))
      (goto-char (point-min))
      (while (and nomatch
		  (re-search-forward
  ...
  '(["跳转到缓冲区" speedbar-edit-line t]
    ...
    ["折叠文件 Tags 标识" speedbar-contract-line
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *.-. "))]
    "----"
    ["剪切(杀死)缓冲区" speedbar-buffer-kill-buffer
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *.[-+?]. "))]
    ["重读缓冲区" speedbar-buffer-revert-buffer
     (save-excursion (beginning-of-line)
		     (looking-at "[0-9]+: *.[-+?]. "))]
    )
...
If TEMP is non-nil, then clicking on a buffer restores the previous display."
  ...
  (let ((bl (buffer-list))
	...
	(let* ((known (string-match speedbar-file-regexp
      ...
      (setq bl (cdr bl)))
    (setq bl (buffer-list))
    (speedbar-insert-separator "草稿缓冲区:")
    (while bl
  ...
  (or (speedbar-item-info-tag-helper)
	...
	(and buffer
	     (speedbar-message "%s%s %S %d %s"
			       ...
			       (or (buffer-file-name buffer) "<无文件>"))))))
  ...
  (or (save-excursion
	(let ((text (speedbar-line-text)))
	  (if (and (get-buffer text)
		   (speedbar-y-or-n-p (format "剪切(杀死)缓冲区 %s? " text)))
	      (kill-buffer text))
	  (speedbar-refresh)))))
