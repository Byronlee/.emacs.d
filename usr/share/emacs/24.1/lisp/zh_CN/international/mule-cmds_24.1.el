...
(defvar describe-language-environment-map
  (let ((map (make-sparse-keymap "语言环境说明")))
    (define-key map
      [Default] `(menu-item ,(purecopy "默认值") describe-specified-language-support))
    map))

(defvar setup-language-environment-map
  (let ((map (make-sparse-keymap "设置语言环境")))
    (define-key map
      [Default] `(menu-item ,(purecopy "默认值") setup-specified-language-environment))
    map))

(defvar set-coding-system-map
  (let ((map (make-sparse-keymap "设置文字编码")))
    ...
    (define-key-after map [set-keyboard-coding-system]
      `(menu-item ,(purecopy "用于键盘") set-keyboard-coding-system
        :help ,(purecopy "How to decode keyboard input")))
    (define-key-after map [set-terminal-coding-system]
      `(menu-item ,(purecopy "用于终端") set-terminal-coding-system
    ...
    (define-key-after map [set-selection-coding-system]
      `(menu-item ,(purecopy "用于 X 选择/剪贴板") set-selection-coding-system
    ...
    (define-key-after map [set-next-selection-coding-system]
      `(menu-item ,(purecopy "用于下一次 X 选择") set-next-selection-coding-system
        :visible (display-selections-p)
        :help ,(purecopy "How to en/decode next selection/clipboard operation")))
    (define-key-after map [set-buffer-process-coding-system]
      `(menu-item ,(purecopy "用于 I/O 子进程") set-buffer-process-coding-system
...
(defvar mule-menu-keymap
  (let ((map (make-sparse-keymap "Mule(多国语言环境)")))
    (define-key-after map [set-language-environment]
      `(menu-item  ,(purecopy "设置语言环境") ,setup-language-environment-map))
    (define-key-after map [separator-mule] menu-bar-separator)

    (define-key-after map [toggle-input-method]
      `(menu-item ,(purecopy "切换输入法") toggle-input-method))
    (define-key-after map [set-input-method]
      `(menu-item ,(purecopy "选择输入法...") set-input-method))
    ...
    (define-key-after map [set-various-coding-system]
      `(menu-item ,(purecopy "设置文字编码") ,set-coding-system-map
		  :enable (default-value 'enable-multibyte-characters)))
    (define-key-after map [view-hello-file]
      `(menu-item ,(purecopy "显示多国语言文本") view-hello-file
    ...
    (define-key-after map [mule-diag]
      `(menu-item ,(purecopy "显示全部 Mule 状态") mule-diag
  ...
  (interactive
   (let ((default (and buffer-file-coding-system
     ...
     (list (read-coding-system
	    (if default
		(format "接下来的命令使用的文字编码: (默认为 %s)" default)
	      "接下来的命令使用的文字编码: ")
	    default))))
  (let* ((keyseq (read-key-sequence
		  (format "用 %s 来执行的命令" coding-system)))
...
character found, or nil if all characters are encodable."
  ...
  (let ((pos (unencodable-char-position (point) (point-max) coding-system)))
    (if pos
	(goto-char (1+ pos))
      (message "后面的字符可以按 %s 进行编码" coding-system))
    pos))
...
e.g., for sending an email message.\n ")
		...
		(princ " ") (princ x))
	      (insert "\n"))
	    (when unsafe
	      (insert (if rejected "其它文字编码"
			"但是, 当中的每一种")
		      " 有冲突的字符无法进行编码:\n")
	      (dolist (coding unsafe)
		(insert (format "  %s 无法对这些进行编码:" (car coding)))
		(let ((i 0)
		      (func1
		       #'(lambda (bufname pos)
  ...
  (let ((describe-map describe-language-environment-map)
	...
	(let ((l parents)
	  ...
	  (while l
	    ...
	    (or map (setq prompt (format "%s 环境" parent)))
	    (if (not map)
		(progn
...
(defgroup leim nil
  "LEIM: 即 Emacs 输入法库 (Libraries of Emacs Input Methods)."
  :group 'mule)
...
instead of the one selected last time.  It is a symbol or a string."
  (setq input-method
	(or input-method
	    current-input-method
	    default-input-method
	    (read-input-method-name "输入法: " nil t)))
  (if (and input-method (symbolp input-method))
      (setq input-method (symbol-name input-method)))
...
See also the variable `input-method-highlight-flag'."
  :type '(choice (const :tag "总是" t) (const :tag "从不" nil)
  ...
  (let ((doc (get-language-info language-name 'documentation)))
	...
	(let ((input-method (get-language-info language-name 'input-method))
	  ...
	  (when (and input-method
	    ...
	    (insert "输入法 (default " (car input-method) ")\n")
...
properties are sticky."
  (interactive
   (list (read-char-by-name "Unicode (名称或16进制)")
  ...
  (cond
   ...
   ((not (integerp character))
    (error "不是一个 Unicode 字符代码: %S" character))
   ((or (< character 0) (> character #x10FFFF))
    (error "不是一个 Unicode 字符代码: 0x%X" character)))
  (if inherit
