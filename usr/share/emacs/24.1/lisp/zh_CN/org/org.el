...
(easy-menu-define org-tbl-menu org-mode-map "Tbl menu"
  '("Tbl"
    ["对齐" org-ctrl-c-ctrl-c :active (org-at-table-p)]
    ["下一区" org-cycle (org-at-table-p)]
    ["上一区" org-shifttab (org-at-table-p)]
    ["下一行" org-return (org-at-table-p)]
    "--"
    ["空白区" org-table-blank-field (org-at-table-p)]
    ["编辑区" org-table-edit-field (org-at-table-p)]
    ["从上面复制区" org-table-copy-down (org-at-table-p)]
    "--"
    ("列"
     ["列左移" org-metaleft (org-at-table-p)]
     ["列右移" org-metaright (org-at-table-p)]
     ["删除列" org-shiftmetaleft (org-at-table-p)]
     ["插入列" org-shiftmetaright (org-at-table-p)])
    ("行"
     ["行上移" org-metaup (org-at-table-p)]
     ["行下移" org-metadown (org-at-table-p)]
     ["删除行" org-shiftmetaup (org-at-table-p)]
     ["插入行" org-shiftmetadown (org-at-table-p)]
     ["区域内的行排序" org-table-sort-lines (org-at-table-p)]
     "--"
     ["插入标题行" org-ctrl-c-minus (org-at-table-p)])
    ("选区"
     ["复制选区" org-copy-special (org-at-table-p)]
     ["剪切选区" org-cut-special (org-at-table-p)]
     ["粘贴选区" org-paste-special (org-at-table-p)]
     ["调整选区" org-table-wrap-region (org-at-table-p)])
    "--"
    ("计算"
     ["设置列公式" org-table-eval-formula (org-at-table-p)]
     ["设置区公式" (org-table-eval-formula '(4)) :active (org-at-table-p) :keys "C-u C-c ="]
     ["编辑公式" org-edit-special (org-at-table-p)]
     "--"
     ["重新计算行" org-table-recalculate (org-at-table-p)]
     ["全部重新计算" (lambda () (interactive) (org-table-recalculate '(4))) :active (org-at-table-p) :keys "C-u C-c *"]
     ["全部重复 (Iterate)" (lambda () (interactive) (org-table-recalculate '(16))) :active (org-at-table-p) :keys "C-u C-u C-c *"]
     "--"
     ["切换重新计算标记" org-table-rotate-recalc-marks (org-at-table-p)]
     "--"
     ["列/区求和" org-table-sum
      (or (org-at-table-p) (org-region-active-p))]
     ["哪一栏?" org-table-current-column (org-at-table-p)])
    ["调试公式"
    ...
    "--"
    ["创建" org-table-create (and (not (org-at-table-p))
				    org-enable-table-editor)]
    ["转换区域" org-table-convert-region (not (org-at-table-p 'any))]
    ["从文件导入" org-table-import (not (org-at-table-p))]
    ["导出到文件" org-table-export (org-at-table-p)]
    "--"
    ["创建/转换 table.el" org-table-create-with-table.el t]))

(easy-menu-define org-org-menu org-mode-map "Org menu"
  '("Org"
    ("显示/隐藏"
     ["循环可见" org-cycle :active (or (bobp) (outline-on-heading-p))]
     ["全局循环可见" org-shifttab :active (not (org-at-table-p))]
     ["简明树..." org-sparse-tree t]
     ["展开上下文" org-reveal t]
     ["显示全部" show-all t]
     "--"
     ["到间接缓冲区的子树" org-tree-to-indirect-buffer t])
    "--"
    ["新标题" org-insert-heading t]
    ("标题导航"
     ["向上" outline-up-heading t]
     ["下一个" outline-next-visible-heading t]
     ["上一个" outline-previous-visible-heading t]
     ["下一个同级" outline-forward-same-level t]
     ["上一个同级" outline-backward-same-level t]
     "--"
     ["跳转" org-goto t])
    ("编辑结构"
     ["子树上移" org-shiftmetaup (not (org-at-table-p))]
     ["子树下移" org-shiftmetadown (not (org-at-table-p))]
     "--"
     ["复制子树"  org-copy-special (not (org-at-table-p))]
     ["剪切子树"  org-cut-special (not (org-at-table-p))]
     ["粘贴子树"  org-paste-special (not (org-at-table-p))]
     "--"
     ["按住 Shift 键时克隆子树" org-clone-subtree-with-time-shift t]
     "--"
     ["复制可见文本"  org-copy-visible t]
     "--"
     ["提升标题" org-metaleft (not (org-at-table-p))]
     ["提升子树" org-shiftmetaleft (not (org-at-table-p))]
     ["下沉标题"  org-metaright (not (org-at-table-p))]
     ["下沉子树"  org-shiftmetaright (not (org-at-table-p))]
     "--"
     ["区域/子区域排序" org-sort  (not (org-at-table-p))]
     "--"
     ["转换到偶数层" org-convert-to-odd-levels t]
     ["转换到偶数/单数层" org-convert-to-oddeven-levels t])
    ("编辑"
     ["强调..." org-emphasize t]
     ["编辑源码样例" org-edit-special t]
     "--"
     ["脚注新建/跳转" org-footnote-action t]
     ["附加的脚注" (org-footnote-action t) :active t :keys "C-u C-c C-x f"])
    ("存档"
     ["存档(默认方式)" org-archive-subtree-default t]
     "--"
     ["移动子树到存档文件" org-advertized-archive-subtree t]
     ["切换存档 ARCHIVE 的 tag 标识" org-toggle-archive-tag t]
     ["移动子树到存档 Archive 的同级" org-archive-to-archive-sibling t]
     )
    "--"
    ("超链接"
     ["保存链接(全局)" org-store-link t]
     ["查找指向这里的链接" org-occur-link-in-agenda-files t]
     ["插入链接" org-insert-link t]
     ["跟从链接" org-open-at-point t]
     "--"
     ["下一个链接" org-next-link t]
     ["上一个链接" org-previous-link t]
     "--"
     ["描述的链接 (Descriptive Links)"
    ...
    ("待办列表"
     ["待办/完成/-" org-todo t]
     ("选择关键词"
      ["下一个关键词" org-shiftright (org-at-heading-p)]
      ["上一个关键词" org-shiftleft (org-at-heading-p)]
      ["补齐关键词" pcomplete (assq :todo-keyword (org-context))]
      ["下一个关键词集合" org-shiftcontrolright (and (> (length org-todo-sets) 1) (org-at-heading-p))]
      ["上一个关键词集合" org-shiftcontrolright (and (> (length org-todo-sets) 1) (org-at-heading-p))])
     ["显示待办树" org-show-todo-tree :active t :keys "C-c / t"]
     ["全局待办列表" org-todo-list :active t :keys "C-c a t"]
     "--"
     ["强制依赖" (customize-variable 'org-enforce-todo-dependencies)
      :selected org-enforce-todo-dependencies :style toggle :active t]
     "树上某个点的设置"
     ["按顺序执行子项目 (Do Children sequentially)" org-toggle-ordered-property :style radio
      :selected (org-entry-get nil "ORDERED")
      :active org-enforce-todo-dependencies :keys "C-c C-x o"]
     ["并行执行子项目 (Do Children parallel)" org-toggle-ordered-property :style radio
      :selected (not (org-entry-get nil "ORDERED"))
      :active org-enforce-todo-dependencies :keys "C-c C-x o"]
     "--"
     ["设置优先级" org-priority t]
     ["优先级上移" org-shiftup t]
     ["优先级下移" org-shiftdown t]
     "--"
     ["收取全部 feed 新邮件" org-feed-update-all t]
     ["转到 feed 收件箱..." org-feed-goto-inbox t]
     ["自定义 feed" (customize-variable 'org-feed-alist) t])
    ("TAG 标识和属性"
     ["设置 Tag 标识" org-set-tags-command t]
     ["更改区域内的 tag 标识" org-change-tag-in-region (org-region-active-p)]
     "--"
     ["设置属性" org-set-property t]
     ["分栏方式查看属性" org-columns t]
     ["插入分栏查看栏目" org-insert-columns-dblock t])
    ("日期和计划"
     ["时间戳" org-time-stamp t]
     ["时间戳(非链接)" org-time-stamp-inactive t]
     ("更改日期"
      ["1 天后" org-shiftright t]
      ["1 天前" org-shiftleft t]
      ["1 ... 后" org-shiftup t]
      ["1 ... 前" org-shiftdown t])
     ["计算时间范围" org-evaluate-time-range t]
     ["计划事项" org-schedule t]
     ["截止时间" org-deadline t]
     "--"
     ["自定义时间格式 (Custom time format)" org-toggle-time-stamp-overlays
      :style radio :selected org-display-custom-times]
     "--"
     ["转到日历" org-goto-calendar t]
     ["日历日期" org-date-from-calendar t]
     "--"
     ["启动/重新启动计时器" org-timer-start t]
     ["暂停/继续计时器" org-timer-pause-or-continue t]
     ["停止计时器" org-timer-pause-or-continue :active t :keys "C-u C-c C-x ,"]
     ["插入计时器字符串" org-timer t]
     ["插入计时器事项" org-timer-item t])
    ("记录工作"
     ["计时开始(Clock in)" org-clock-in :active t :keys "C-c C-x C-i"]
     ["转换任务" (lambda () (interactive) (org-clock-in '(4))) :active t :keys "C-u C-c C-x C-i"]
     ["计时结束(Clock out)" org-clock-out t]
     ["取消计时" org-clock-cancel t]
     "--"
     ["标记为默认任务" org-clock-mark-default-task t]
     ["计时开始(Clock in), 标记为默认" (lambda () (interactive) (org-clock-in '(16))) :active t :keys "C-u C-u C-c C-x C-i"]
     ["转到正在计时的时钟" org-clock-goto t]
     "--"
     ["显示次数" org-clock-display t]
     ["创建时间表" org-clock-report t]
     "--"
     ["记录完成时间"
      (progn (setq org-log-done (not org-log-done))
	     (message "转换到 %s 会 %s 记录时间戳"
		      (car org-done-keywords)
		      (if org-log-done "automatically" "not")))
      :style toggle :selected org-log-done])
    "--"
    ["行程命令..." org-agenda t]
    ["设置限制锁" org-agenda-set-restriction-lock t]
    ("行程的文件列表")
    ("单独查看当前文件"
     ["树状显示待办事项"  org-show-todo-tree t]
     ["检查截止时间" org-check-deadlines t]
     ["时间线" org-timeline t]
     ["Tag 标识/属性树" org-match-sparse-tree t])
    "--"
    ["导出/发布..." org-export t]
    ("LaTeX"
     ["Org CDLaTeX 模式" org-cdlatex-mode :style toggle
      :selected org-cdlatex-mode]
     ["插入嵌套(Environment)" cdlatex-environment (fboundp 'cdlatex-environment)]
     ["插入数字符号" cdlatex-math-symbol (fboundp 'cdlatex-math-symbol)]
     ["修改数字符号" org-cdlatex-math-modify
      (org-inside-LaTeX-fragment-p)]
     ["插入引用" org-reftex-citation t]
     "--"
     ["BEAMER 模板" (progn (require 'org-beamer)
				   (org-insert-beamer-options-template)) t])
    "--"
    ("MobileOrg"
     ["推送文件和查看" org-mobile-push t]
     ["获取已抓取和已进行过 Flag 标定的" org-mobile-pull t]
     ["查找已进行 Flag 标定的任务" (org-agenda nil "?") :active t :keys "C-c a ?"]
     "--"
     ["设置" (progn (require 'org-mobile) (customize-group 'org-mobile)) t])
    "--"
    ("文档"
     ["显示版本" org-version t]
     ["Info 说明文档" org-info t])
    ("自定义"
     ["浏览 Org 分组" org-customize t]
     "--"
     ["展开这个菜单" org-create-customize-menu
      (fboundp 'customize-menu-create)])
    ["发送缺陷报告" org-submit-bug-report t]
    "--"
    ("刷新/重新加载"
     ["刷新设置当前缓冲区" org-mode-restart t]
     ["(在更新后)重新加载 Org" org-reload t]
     ["重新加载未编译的 Org" (org-reload t) :active t :keys "C-u C-c C-x r"])
    ))
...
(defun org-install-agenda-files-menu ()
  (let ((bl (buffer-list)))
	...
	(easy-menu-change
	 ...
	 (append
	  (list
	   ["编辑文件列表" (org-edit-agenda-file-list) t]
	   ["添加/移动当前文件到列表最前面" org-agenda-file-to-front t]
	   ["从列表中移除当前文件" org-remove-file t]
	   ["循环通过行程文件" org-cycle-agenda-files t]
	   ["在所有行程中发生" org-occur-in-agenda-files t]
	   "--")
  ...
  (if (fboundp 'customize-menu-create)
	...
	(easy-menu-change
	 ...
	 `(["Browse Org group" org-customize t]
	   ...
	   ["设置" Custom-set t]
	   ["保存" Custom-save t]
	   ["重设为当前值" Custom-reset-current t]
	   ["重设为已保存的" Custom-reset-saved t]
	   ["重设为标准设置" Custom-reset-standard t]))
