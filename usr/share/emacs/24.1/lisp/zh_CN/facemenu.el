...
(let ((map facemenu-menu))
  ...
  (define-key map [ra] (list 'menu-item (purecopy "移除文本属性")
			     'facemenu-remove-all
			     :enable 'mark-active))
  (define-key map [rm] (list 'menu-item (purecopy "移除外观属性")
...
and excludes grayscale colors."
  :type '(choice (const :tag "未排序的" nil)
		 (const :tag "颜色名" name)
		 (const :tag "红-绿-蓝" rgb)
		 (cons :tag "RGB 调色板距离"
		       (const :tag "色彩距离" rgb-dist)
		       (color :tag "来源颜色名称"))
		 (const :tag "HUE 色相饱和度数值" hsv)
		 (cons :tag "HSV 调色盘距离"
		       (const :tag "色彩距离" hsv-dist)
		       (color :tag "来源颜色名称")))
  :group 'facemenu
  :version "24.1")
