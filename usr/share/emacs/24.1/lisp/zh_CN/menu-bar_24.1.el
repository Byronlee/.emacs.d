...
(defvar menu-bar-file-menu
  (let ((menu (make-sparse-keymap "文件")))
    ...
    (define-key menu [new-window-on-right]
      `(menu-item ,(purecopy "在右侧新建窗格") split-window-right
  ...
  (let ((menu (make-sparse-keymap "编辑")))
    ...
    (if (featurep 'ns)
        (define-key menu [spell]
          `(menu-item ,(purecopy "拼写") ispell-menu-map)))
    ...
    (define-key menu (if (featurep 'ns) [select-paste]
      ...
      `(menu-item ,(purecopy (if (featurep 'ns) "选择并粘贴"
    ...
    (define-key menu [copy]
      ...
      ;; under X (for GNUstep).
      `(menu-item ,(purecopy "复制") ,(if (featurep 'ns)
...
(defvar menu-bar-custom-menu
  (let ((menu (make-sparse-keymap "自定义")))
    ...
    (define-key menu [customize-themes]
      `(menu-item ,(purecopy "自定义主题") customize-themes
...
mail status in mode line"))
    ...
    (if (and (boundp 'menu-bar-showhide-tool-bar-menu)
        ...
        (define-key menu [showhide-tool-bar]
          `(menu-item ,(purecopy "工具栏") ,menu-bar-showhide-tool-bar-menu
                      :visible (display-graphic-p)))
...
(defvar menu-bar-line-wrapping-menu
  (let ((menu (make-sparse-keymap "换行")))
    ...
    (define-key menu [word-wrap]
      `(menu-item
	,(purecopy "单词换行(可见行模式) [Word Wrap (Visual Line mode)]")
	(lambda ()
	  (interactive)
...
(defvar menu-bar-options-menu
  (let ((menu (make-sparse-keymap "选项")))
    ...
    (define-key menu [mule]
      ...
      `(menu-item ,(purecopy "多国语言环境(Mule)") ,mule-menu-keymap
...
(defvar menu-bar-describe-menu
  (let ((menu (make-sparse-keymap "说明")))
    ...
    (define-key menu [describe-package]
      `(menu-item ,(purecopy "软件包说明...") describe-package
  ...
  (let ((menu (make-sparse-keymap "帮助")))
    ...
    (and (featurep 'ns)
         ...
         (define-key menu [info-panel]
           `(menu-item ,(purecopy "关于 Emacs...") ns-do-emacs-info-panel)))
    menu))
...
(define-key global-map [menu-bar edit]
  (cons (purecopy "编辑") menu-bar-edit-menu))
(define-key global-map [menu-bar file]
  (cons (purecopy "文件") menu-bar-file-menu))
...
(if (and (featurep 'ns)
    ...
    (define-key global-map [menu-bar help-menu]
      (cons (purecopy "信息") menu-bar-help-menu))
