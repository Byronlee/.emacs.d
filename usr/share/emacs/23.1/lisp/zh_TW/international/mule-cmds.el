...
;;; 2013-05-19 22:08:00 CST translated
...
(define-key-after set-coding-system-map [set-file-name-coding-system]
  '(menu-item "用于文件名" set-file-name-coding-system
...
(define-key-after set-coding-system-map [set-selection-coding-system]
  '(menu-item "用于 X 選擇/剪貼板" set-selection-coding-system
...
(define-key-after set-coding-system-map [set-next-selection-coding-system]
  '(menu-item "用于下一次 X 選擇" set-next-selection-coding-system
	      :visible (display-selections-p)
...
(define-key-after set-coding-system-map [set-buffer-process-coding-system]
  '(menu-item "用于 I/O 子進程" set-buffer-process-coding-system
...
(define-key setup-language-environment-map
  [Default] '(menu-item "默認值" setup-specified-language-environment))

(define-key describe-language-environment-map
  [Default] '(menu-item "默認值" describe-specified-language-support))
...
using `read-char-by-name'."
  (interactive (list (read-char-by-name "Unicode (名稱或16進制)")))
  (if (stringp arg)
      (setq arg (string-to-number arg 16)))
  (cond
   ((not (integerp arg))
    (error "不是一个 Unicode 字符代碼: %S" arg))
   ((or (< arg 0) (> arg #x10FFFF))
    (error "不是一个 Unicode 字符代碼: 0x%X" arg)))
