...
(defcustom display-time-world-list
  ...
  (let (gmt nyt)
    (set-time-zone-rule "美国/纽约")
    (setq nyt (format-time-string "%z"))
    (set-time-zone-rule "欧洲/伦敦")
...
(defcustom display-time-world-buffer-name "*世界时钟*"
...
make the mail indicator stand out on a color display."
  ...
  :group 'display-time
  :version "22.1"
  :type '(choice (const :tag "无" nil) face))
...
(defcustom display-time-mail-string "邮件"
...
This runs the normal hook `display-time-hook' after each update."
  ...
  (if display-time-mode
	...
	(display-time-update)
	;; When you get new mail, clear "邮件" from the mode line.
