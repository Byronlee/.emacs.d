...
(defvar menu-bar-file-menu
  (let ((menu (make-sparse-keymap "文件")))
    ...
    (bindings--define-key menu [new-window-on-right]
      '(menu-item "在右侧新建窗格" split-window-right
...
(defvar menu-bar-i-search-menu
  (let ((menu (make-sparse-keymap "渐进式搜索")))
...
(defvar menu-bar-search-menu
  (let ((menu (make-sparse-keymap "搜索")))
    ...
    (bindings--define-key menu [i-search]
      `(menu-item "渐进式搜索" ,menu-bar-i-search-menu))
...
(defvar menu-bar-replace-menu
  (let ((menu (make-sparse-keymap "替换")))
...
(defvar menu-bar-goto-menu
  (let ((menu (make-sparse-keymap "转到")))
...
(defvar menu-bar-edit-menu
  (let ((menu (make-sparse-keymap "编辑")))
    ...
    (bindings--define-key menu [goto]
      `(menu-item "转到" ,menu-bar-goto-menu))

    (bindings--define-key menu [replace]
      `(menu-item "替换" ,menu-bar-replace-menu))
    ...
    (bindings--define-key menu (if (featurep 'ns) [select-paste]
      ...
      `(menu-item ,(if (featurep 'ns) "选择并粘贴"
                     "从剪贴板(即 Kill 菜单)进行粘贴") yank-menu
...
(defvar menu-bar-custom-menu
  (let ((menu (make-sparse-keymap "自定义")))
    ...
    (bindings--define-key menu [customize-themes]
      '(menu-item "自定义主题" customize-themes
...
(defvar menu-bar-showhide-fringe-ind-menu
  (let ((menu (make-sparse-keymap "缓冲区包边")))
...
(defvar menu-bar-showhide-fringe-menu
  (let ((menu (make-sparse-keymap "包边")))

    (bindings--define-key menu [showhide-fringe-ind]
      `(menu-item "缓冲区边界" ,menu-bar-showhide-fringe-ind-menu
                  :visible (display-graphic-p)
...
(defvar menu-bar-showhide-scroll-bar-menu
  (let ((menu (make-sparse-keymap "滑块")))
...
(when (featurep 'move-toolbar)
  (defvar menu-bar-showhide-tool-bar-menu
    (let ((menu (make-sparse-keymap "工具栏")))
...
(defvar menu-bar-showhide-menu
  (let ((menu (make-sparse-keymap "显示/隐藏")))
...
mail status in mode line"))
    ...
    (if (and (boundp 'menu-bar-showhide-tool-bar-menu)
        ...
        (bindings--define-key menu [showhide-tool-bar]
          `(menu-item "工具栏" ,menu-bar-showhide-tool-bar-menu
                      :visible (display-graphic-p)))
      ;; else not tool bar that can move.
      (bindings--define-key menu [showhide-tool-bar]
        '(menu-item "工具栏" toggle-tool-bar-mode-from-frame
                    :help "打开/关闭工具栏"
                    :visible (display-graphic-p)
                    :button
                    (:toggle . (menu-bar-positive-p
...
(defvar menu-bar-line-wrapping-menu
  (let ((menu (make-sparse-keymap "换行")))

    (bindings--define-key menu [word-wrap]
      `(menu-item "单词换行(可见行模式) [Word Wrap (Visual Line mode)]"
                  ,(lambda ()
                     (interactive)
                     (unless visual-line-mode
                       (visual-line-mode 1))
                     (message "启用可见行模式 (Visual-Line mode enabled)"))
    ...
    (bindings--define-key menu [truncate]
      `(menu-item "长行换行 (Truncate Long Lines)"
                  ,(lambda ()
                     (interactive)
    ...
    (bindings--define-key menu [window-wrap]
      `(menu-item "在窗格边缘换行 (Wrap at Window Edge)"
                  ,(lambda () (interactive)
                     (if visual-line-mode (visual-line-mode 0))
                     (setq word-wrap nil)
...
(defvar menu-bar-options-menu
  (let ((menu (make-sparse-keymap "选项")))
    (bindings--define-key menu [customize]
      `(menu-item "自定义 Emacs" ,menu-bar-custom-menu))

    (bindings--define-key menu [package]
      '(menu-item "管理 Emacs 软件包" package-list-packages
    ...
    (bindings--define-key menu [menu-set-font]
      '(menu-item "设置默认字体..." menu-set-font
    ...
    (bindings--define-key menu [showhide]
      `(menu-item "显示/隐藏" ,menu-bar-showhide-menu))
    ...
    (bindings--define-key menu [mule]
      ...
      `(menu-item "多国语言环境(Mule)" ,mule-menu-keymap
    ...
    ;;  `(menu-item "参数" ,menu-bar-preferences-menu
    ;;	      :help "切换重要的全局选项"))
    ...
    (bindings--define-key menu [cua-mode]
      (menu-bar-make-mm-toggle
       cua-mode
       "使用 CUA 按键(使用 C-x/C-c/C-v 进行剪切/粘贴)"
...
(defvar menu-bar-games-menu
  (let ((menu (make-sparse-keymap "游戏")))
    ...
    (bindings--define-key menu [land]
      '(menu-item "地标(landmark)" landmark
...
(defvar menu-bar-tools-menu
  (let ((menu (make-sparse-keymap "工具")))
    ...
    (bindings--define-key menu [games]
      `(menu-item "游戏" ,menu-bar-games-menu))
...
(defvar menu-bar-describe-menu
  (let ((menu (make-sparse-keymap "说明")))
    ...
    (bindings--define-key menu [describe-package]
      '(menu-item "软件包说明..." describe-package
...
(defvar menu-bar-search-documentation-menu
  (let ((menu (make-sparse-keymap "搜索文档")))
...
(defvar menu-bar-manuals-menu
  (let ((menu (make-sparse-keymap "更多手册")))
...
(defvar menu-bar-help-menu
  (let ((menu (make-sparse-keymap "帮助")))
    ...
    (bindings--define-key menu [describe]
      `(menu-item "说明" ,menu-bar-describe-menu))
    (bindings--define-key menu [search-documentation]
      `(menu-item "搜索文档" ,menu-bar-search-documentation-menu))
    (bindings--define-key menu [sep1]
      menu-bar-separator)
    ...
    ;; FIXME? There already is an "关于 Emacs" (sans ...) entry in the Help menu.
    (and (featurep 'ns)
         (not (eq system-type 'darwin))
         (bindings--define-key menu [info-panel]
           '(menu-item "关于 Emacs..." ns-do-emacs-info-panel)))
    menu))
...
(bindings--define-key global-map [menu-bar edit]
  (cons "编辑" menu-bar-edit-menu))
(bindings--define-key global-map [menu-bar file]
  (cons "文件" menu-bar-file-menu))

;; Put "帮助" menu at the end, or Info at the front.
;; If running under GNUstep, "帮助" is moved and renamed "信息" (see below).
(if (and (featurep 'ns)
         (not (eq system-type 'darwin)))
    (bindings--define-key global-map [menu-bar help-menu]
      (cons "信息" menu-bar-help-menu))
  (define-key-after global-map [menu-bar help-menu]
    (cons (purecopy "帮助") menu-bar-help-menu)))
