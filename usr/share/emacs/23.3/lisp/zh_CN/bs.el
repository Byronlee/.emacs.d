...
(defcustom bs-attributes-list
  '((""       1   1 left  bs--get-marked-string)
    ...
    (""       1   1 left  " ")
    ("大小"   8   8 right bs--get-size-string)
    (""       1   1 left  " ")
    ("模式"   12 12 right bs--get-mode-name)
    (""       2   2 left  "  ")
    ("文件"   12 12 left  bs--get-file-name)
    (""       2   2 left  "  "))
...
(defcustom bs-sort-functions
  '(("按名称"     bs--sort-by-name     "Buffer" region)
    ("按大小"     bs--sort-by-size     "大小"   region)
    ("按模式"     bs--sort-by-mode     "模式"   region)
    ("按文件名" bs--sort-by-filename "文件"   region)
    ("没有条件"  nil                  nil      nil))
...
(defcustom bs-default-sort-name "没有条件"
...
by buffer configuration `bs-cycle-configuration-name'."
  ...
  (let ((bs--buffer-coming-from (current-buffer))
	...
	(bs-message-without-log "上一个缓冲区: %s"
