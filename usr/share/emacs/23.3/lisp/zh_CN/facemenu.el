...
(defvar facemenu-foreground-menu
  (let ((map (make-sparse-keymap "前景色")))
    (define-key map "o" (cons (purecopy "其它...") 'facemenu-set-foreground))
    map)
...
(defvar facemenu-background-menu
  (let ((map (make-sparse-keymap "背景色")))
    (define-key map "o" (cons (purecopy "其它...") 'facemenu-set-background))
    map)
	 ...
	 (completer
	  (lambda (string pred all-completions)
		...
		(or (all-completions string color-list pred)
		  ...
		  (if (color-defined-p string)
		      string)))))
	 (col (completing-read (or prompt "颜色: ") completer nil t)))
    (if (equal "" col)
	nil
      col)))
