...
(define-key-after mule-menu-keymap [mule-diag]
  '(menu-item "显示全部 Mule 状态" mule-diag
...
(define-key-after set-coding-system-map [set-selection-coding-system]
  '(menu-item "用于 X 选择/剪贴板" set-selection-coding-system
...
(define-key-after set-coding-system-map [set-next-selection-coding-system]
  '(menu-item "用于下一次 X 选择" set-next-selection-coding-system
	      :visible (display-selections-p)
...
(define-key-after set-coding-system-map [set-buffer-process-coding-system]
  '(menu-item "用于 I/O 子进程" set-buffer-process-coding-system
...
(define-key setup-language-environment-map
  [Default] '(menu-item "默认值" setup-specified-language-environment))

(define-key describe-language-environment-map
  [Default] '(menu-item "默认值" describe-specified-language-support))
...
using `read-char-by-name'."
  (interactive (list (read-char-by-name "Unicode (名称或16进制)")))
  (if (stringp arg)
      (setq arg (string-to-number arg 16)))
  (cond
   ((not (integerp arg))
    (error "不是一个 Unicode 字符代码: %S" arg))
   ((or (< arg 0) (> arg #x10FFFF))
    (error "不是一个 Unicode 字符代码: 0x%X" arg)))
