...
;;; 2013-05-19 22:10:08 CST translated
...
(defvar menu-bar-file-menu
  (let ((menu (make-sparse-keymap "檔案")))
    ...
    (define-key menu [delete-this-frame]
      `(menu-item ,(purecopy "刪除框架") delete-frame
    ...
    (define-key menu [make-frame-on-display]
      `(menu-item ,(purecopy "在其它控製台新建框架...") make-frame-on-display
    ...
    (define-key menu [recover-session]
      `(menu-item ,(purecopy "恢復崩潰的會話") recover-session
                  :enable
    ...
    (define-key menu [revert-buffer]
      `(menu-item ,(purecopy "重讀緩沖區") revert-buffer
...
(defvar menu-bar-search-menu
  (let ((menu (make-sparse-keymap "搜索")))
    ...
    (define-key menu [i-search]
      `(menu-item ,(purecopy "漸進式搜索") ,menu-bar-i-search-menu))
    (define-key menu [separator-tag-isearch]
      menu-bar-separator)

    (define-key menu [tags-continue]
      `(menu-item ,(purecopy "繼續搜索 Tag 標識") tags-loop-continue
                  :help ,(purecopy "Continue last tags search operation")))
    (define-key menu [tags-srch]
      `(menu-item ,(purecopy "搜索帶 Tag 標識的檔案...") tags-search
...
(defvar menu-bar-replace-menu
  (let ((menu (make-sparse-keymap "替換")))
    ...
    (define-key menu [tags-repl]
      `(menu-item ,(purecopy "在帶 Tag 標識的檔案中進行替換...") tags-query-replace
        :help ,(purecopy
...
(defvar menu-bar-goto-menu
  (let ((menu (make-sparse-keymap "轉到")))
    ...
    (define-key menu [set-tags-name]
      `(menu-item ,(purecopy "設置 Tag 標識檔案名...") visit-tags-table
    ...
    (define-key menu [end-of-buf]
      `(menu-item ,(purecopy "轉到緩沖區末尾") end-of-buffer))
    (define-key menu [beg-of-buf]
      `(menu-item ,(purecopy "轉到緩沖區開頭") beginning-of-buffer))
    (define-key menu [go-to-pos]
      `(menu-item ,(purecopy "轉到緩沖區位置...") goto-char
                  :help ,(purecopy "Read a number N and go to buffer position N")))
    (define-key menu [go-to-line]
      `(menu-item ,(purecopy "轉到某行...") goto-line
...
(defvar menu-bar-edit-menu
  (let ((menu (make-sparse-keymap "編輯")))
    ...
    (define-key menu [props]
      `(menu-item ,(purecopy "文本屬性") facemenu-menu))

    ;; ns-win.el said: Add spell for platform consistency.
    (if (featurep 'ns)
        (define-key menu [spell]
          `(menu-item ,(purecopy "拼寫") ispell-menu-map)))
    ...
    (define-key menu [bookmark]
      `(menu-item ,(purecopy "書簽") menu-bar-bookmark-map))
    ...
    (define-key menu [copy]
      ...
      ;; under X (for GNUstep).
      `(menu-item ,(purecopy "復製") ,(if (featurep 'ns)
...
(defvar menu-bar-custom-menu
  (let ((menu (make-sparse-keymap "自定義")))
    ...
    (define-key menu [customize-themes]
      `(menu-item ,(purecopy "自定義主題") customize-themes
...
(defvar menu-bar-showhide-fringe-ind-menu
  (let ((menu (make-sparse-keymap "緩沖區包邊")))
    ...
    (define-key menu [customize]
      `(menu-item ,(purecopy "其它(自定義) [Other (Customize)]")
    ...
    (define-key menu [right]
      `(menu-item ,(purecopy "在右側包邊內 (In Right Fringe)") menu-bar-showhide-fringe-ind-right
    ...
    (define-key menu [left]
      `(menu-item ,(purecopy "在左側包邊內 (In Left Fringe)") menu-bar-showhide-fringe-ind-left
...
(defvar menu-bar-showhide-fringe-menu
  (let ((menu (make-sparse-keymap "包邊")))
    ...
    (define-key menu [showhide-fringe-ind]
      `(menu-item ,(purecopy "緩沖區邊界") ,menu-bar-showhide-fringe-ind-menu
                  :visible (display-graphic-p)
    ...
    (define-key menu [customize]
      `(menu-item ,(purecopy "自定義包邊") menu-bar-showhide-fringe-menu-customize
                  :help ,(purecopy "Detailed customization of fringe")
                  :visible (display-graphic-p)))

    (define-key menu [default]
      `(menu-item ,(purecopy "默認值 (Default)") menu-bar-showhide-fringe-menu-customize-reset
                  :help ,(purecopy "Default width fringe on both left and right side")
                  :visible (display-graphic-p)
                  :button (:radio . (eq fringe-mode nil))))

    (define-key menu [right]
      `(menu-item ,(purecopy "在右側 (On the Right)") menu-bar-showhide-fringe-menu-customize-right
                  :help ,(purecopy "Fringe only on the right side")
                  :visible (display-graphic-p)
                  :button (:radio . (equal fringe-mode '(0 . nil)))))

    (define-key menu [left]
      `(menu-item ,(purecopy "在左側 (On the Left)") menu-bar-showhide-fringe-menu-customize-left
                  :help ,(purecopy "Fringe only on the left side")
                  :visible (display-graphic-p)
                  :button (:radio . (equal fringe-mode '(nil . 0)))))

    (define-key menu [none]
      `(menu-item ,(purecopy "無 (None)") menu-bar-showhide-fringe-menu-customize-disable
                  :help ,(purecopy "關閉包邊 (Turn off fringe)")
                  :visible (display-graphic-p)
...
(defvar menu-bar-showhide-scroll-bar-menu
  (let ((menu (make-sparse-keymap "滑塊")))
    ...
    (define-key menu [right]
      `(menu-item ,(purecopy "在右側 (On the Right)")
    ...
    (define-key menu [left]
      `(menu-item ,(purecopy "在左側 (On the Left)")
    ...
    (define-key menu [none]
      `(menu-item ,(purecopy "無 (None)")
                  menu-bar-no-scroll-bar
                  :help ,(purecopy "關閉滑塊 (Turn off scroll-bar)")
                  :visible (display-graphic-p)
                  :button (:radio . (eq (cdr (assq 'vertical-scroll-bars
...
mail status in mode line"))
    ...
    (define-key menu [showhide-fringe]
      `(menu-item ,(purecopy "包邊") ,menu-bar-showhide-fringe-menu
                  :visible (display-graphic-p)))

    (define-key menu [showhide-scroll-bar]
      `(menu-item ,(purecopy "滑塊") ,menu-bar-showhide-scroll-bar-menu
                  :visible (display-graphic-p)))
    ...
    (define-key menu [menu-bar-mode]
      `(menu-item ,(purecopy "菜單列") toggle-menu-bar-mode-from-frame
                  :help ,(purecopy "打開/關閉菜單列")
                  :button
...
(defvar menu-bar-line-wrapping-menu
  (let ((menu (make-sparse-keymap "換行")))
    ...
    (define-key menu [word-wrap]
      `(menu-item
	,(purecopy "單詞換行(可見行模式) [Word Wrap (Visual Line mode)]")
	(lambda ()
	  (interactive)
	  (unless visual-line-mode
	    (visual-line-mode 1))
	  (message ,(purecopy "啟用可見行模式 (Visual-Line mode enabled)")))
    ...
    (define-key menu [truncate]
      `(menu-item ,(purecopy "長行換行 (Truncate Long Lines)")
                  (lambda ()
                    (interactive)
    ...
    (define-key menu [window-wrap]
      `(menu-item ,(purecopy "在窗格邊緣換行 (Wrap at Window Edge)")
                  (lambda () (interactive)
                    (if visual-line-mode (visual-line-mode 0))
                    (setq word-wrap nil)
...
(defvar menu-bar-options-menu
  (let ((menu (make-sparse-keymap "選項")))
    ...
    (define-key menu [save]
      `(menu-item ,(purecopy "保存選項") menu-bar-options-save
    ...
    (define-key menu [menu-set-font]
      `(menu-item ,(purecopy "設置默認字體...") menu-set-font
    ...
    (define-key menu [mule]
      ...
      `(menu-item ,(purecopy "多國語言環境(Mule)") ,mule-menu-keymap
    ...
    ;;  `(menu-item ,(purecopy "參數") ,menu-bar-preferences-menu
    ;;	      :help ,(purecopy "切換重要的全局選項")))
...
(defvar menu-bar-games-menu
  (let ((menu (make-sparse-keymap "游戲")))
    ...
    (define-key menu [tetris]
      `(menu-item ,(purecopy "堆積木(tetris)")  tetris
    ...
    (define-key menu [snake]
      `(menu-item ,(purecopy "貪食蛇(snake)")  snake
    ...
    (define-key menu [hanoi]
      `(menu-item ,(purecopy "漢諾塔(hanoi)") hanoi
...
(defvar menu-bar-encryption-decryption-menu
  (let ((menu (make-sparse-keymap "加密/解密")))
    (define-key menu [insert-keys]
      `(menu-item ,(purecopy "插入密鑰") epa-insert-keys
    ...
    (define-key menu [import-keys-region]
      `(menu-item ,(purecopy "從區域導入密鑰") epa-import-keys-region
                  :help ,(purecopy "Import public keys from the current region")))

    (define-key menu [import-keys]
      `(menu-item ,(purecopy "從檔案導入密鑰...") epa-import-keys
    ...
    (define-key menu [verify-region]
      `(menu-item ,(purecopy "驗証區域") epa-verify-region
    ...
    (define-key menu [verify-file]
      `(menu-item ,(purecopy "驗証檔案...") epa-verify-file
...
(defvar menu-bar-tools-menu
  (let ((menu (make-sparse-keymap "工具")))
    ...
    (define-key menu [games]
      `(menu-item ,(purecopy "游戲") ,menu-bar-games-menu))
    ...
    (define-key menu [calendar]
      `(menu-item ,(purecopy "日歷(calendar)") calendar
    ...
    (define-key menu [directory-search]
      `(menu-item ,(purecopy "目錄搜索") eudc-tools-menu))
    ...
    (define-key menu [gnus]
      `(menu-item ,(purecopy "查看網絡新聞(Gnus)") gnus
    ...
    (define-key menu [epatch]
      `(menu-item ,(purecopy "應用補丁") menu-bar-epatch-menu))
    ...
    (define-key menu [grep]
      `(menu-item ,(purecopy "搜索檔案(Grep)...") grep
...
(defvar menu-bar-describe-menu
  (let ((menu (make-sparse-keymap "說明")))
    ...
    (define-key menu [list-keybindings]
      `(menu-item ,(purecopy "列出熱鍵綁定") describe-bindings
    ...
    (define-key menu [describe-package]
      `(menu-item ,(purecopy "軟體包說明...") describe-package
...
key, a click, or a menu-item")))
    (define-key menu [describe-mode]
      `(menu-item ,(purecopy "緩沖區模式說明") describe-mode
...
(defvar menu-bar-search-documentation-menu
  (let ((menu (make-sparse-keymap "搜索文檔")))
    ...
    (define-key menu [search-documentation-strings]
      `(menu-item ,(purecopy "搜索文檔字符串...") apropos-documentation
                  :help
    ...
    (define-key menu [lookup-command-in-manual]
      `(menu-item ,(purecopy "在用戶手冊中尋找指令...") Info-goto-emacs-command-node
                  :help ,(purecopy "Display manual section that describes a command")))
    (define-key menu [lookup-key-in-manual]
      `(menu-item ,(purecopy "在用戶手冊中尋找關鍵詞...") Info-goto-emacs-key-command-node
    ...
    (define-key menu [lookup-subject-in-emacs-manual]
      `(menu-item ,(purecopy "在用戶手冊中尋找主題...") emacs-index-search
...
(defvar menu-bar-manuals-menu
  (let ((menu (make-sparse-keymap "更多手冊")))
    ...
    (define-key menu [other-manuals]
      `(menu-item ,(purecopy "其它全部 Info 手冊") Info-directory
...
(defvar menu-bar-help-menu
  (let ((menu (make-sparse-keymap "幫助")))
    ...
    (define-key menu [search-documentation]
      `(menu-item ,(purecopy "搜索文檔") ,menu-bar-search-documentation-menu))
    (define-key menu [sep1]
      menu-bar-separator)
    (define-key menu [emacs-psychotherapist]
      `(menu-item ,(purecopy "Emacs 心理醫生(doctor)") doctor
                  :help ,(purecopy "Our doctor will help you feel better")))
    (define-key menu [send-emacs-bug-report]
      `(menu-item ,(purecopy "發送缺陷報告...") report-emacs-bug
    ...
    (define-key menu [emacs-faq]
      `(menu-item ,(purecopy "Emacs 常見問題 (FAQ)") view-emacs-FAQ
    ...
    (and (featurep 'ns)
         ...
         (define-key menu [info-panel]
           `(menu-item ,(purecopy "關于 Emacs...") ns-do-emacs-info-panel)))
    menu))

(define-key global-map [menu-bar tools]
  (cons (purecopy "工具") menu-bar-tools-menu))
(define-key global-map [menu-bar buffer]
  (cons (purecopy "緩沖區") global-buffers-menu-map))
(define-key global-map [menu-bar options]
  (cons (purecopy "選項") menu-bar-options-menu))
(define-key global-map [menu-bar edit]
  (cons (purecopy "編輯") menu-bar-edit-menu))
(define-key global-map [menu-bar file]
  (cons (purecopy "檔案") menu-bar-file-menu))
...
(if (and (featurep 'ns)
    ...
    (define-key global-map [menu-bar help-menu]
      (cons (purecopy "迅息") menu-bar-help-menu))
...
(dolist (map (list minibuffer-local-map
  ...
  (define-key map [menu-bar minibuf]
    (cons (purecopy "小緩沖") (make-sparse-keymap "小緩沖"))))
