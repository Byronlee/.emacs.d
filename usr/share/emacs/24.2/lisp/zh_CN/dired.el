...
If nil, `dired-listing-switches' is used."
   :group 'dired
   :type '(choice (const :tag "使用 dired-listing-switches" nil)
                  (string :tag "开关")))
...
For more details, see Info node `(emacs)ls in Lisp'."
  :group 'dired
  :type '(choice (const :tag "检查是否支持 --dired" unspecified)
                 (const :tag "不使用 --dired" nil)
                 (other :tag "使用 --dired" t)))
...
A value of t means move to first file."
  :type '(choice (const :tag "移动到子目录" nil)
		 (const :tag "移动到第一个" t)
		 regexp)
  :group 'dired)
...
see `dired-use-ls-dired' for more details.")
	...
	(insert "  通配符 " (file-name-nondirectory dir) "\n")))))
  ...
  (let ((change (next-single-property-change (point) 'dired-filename nil eol)))
    (cond
     ...
     ((re-search-forward dired-permission-flags-regexp eol t)
      ...
      (if raise-error
	  (error "无法识别的行! 请检查 directory-listing-before-filename-regexp"))
      (beginning-of-line)
      nil)
     (raise-error
      (error "当前行没有文件")))))
...
(defun dired-move-to-end-of-filename (&optional no-error)
  ...
  (if (get-text-property (point) 'dired-filename)
	...
	(if symlink
	  ...
	  (error "%s" (if hidden
		     (substitute-command-keys
		      "文件行是隐藏的，输入 \\[dired-hide-subdir] 取消隐藏")
		   "当前行没有文件")))
      (if (eq opoint (point))
	  nil
	(point)))))
...
Optional argument means return a file name relative to `default-directory'."
  (let ((here (point))
	(alist (or dired-subdir-alist
		   ...
		   (error "%s 中没有子目录 subdir-alist 列表" (current-buffer))))
	elt dir)
    (while alist
      (setq elt (car alist)
	    dir (car elt)
...
Anything else means ask for each directory."
  :type '(choice :tag "删除非空的目录"
		 (const :tag "Yes--是" always)
		 (const :tag "No--不，只删除空目录" nil)
		 (const :tag "确认每个目录" t)
		 (const :tag "只确认每个顶目录" top))
  :group 'dired)
...
(defun dired-internal-do-deletions (l arg &optional trash)
  ...
  (let* ((files (mapcar (function car) l))
    ...
    (if (dired-mark-pop-up
	 "*删除*" 'delete files dired-deletion-confirmer
	 (format "%s %s "
		 (if trashing "回收站" "删除")
...
object files--just `.o' will mark more than you might think."
  (interactive
   (list (dired-read-regexp (concat (if current-prefix-arg "取消标记" "标记")
				    " 文件(表达式): "))
...
`.' and `..' are never flagged."
  (interactive (list (dired-read-regexp "Flag 标定为删除(正则表达): ")))
...
With prefix argument, unmark or unflag all those files."
  ...
  (let ((dired-marker-char (if unflag-p ?\040 dired-marker-char)))
    (dired-mark-if (looking-at dired-re-sym) "符号链接")))
...
With prefix argument, unmark or unflag all those files."
  (interactive "P")
  (let ((dired-marker-char (if unflag-p ?\040 dired-marker-char)))
    (dired-mark-if (looking-at dired-re-exe) "可执行文件")))
...
OLD and NEW are both characters used to mark files."
  (interactive
   (let* ((cursor-in-echo-area t)
	  (old (progn (message "更改(旧标记): ") (read-char)))
	  (new (progn (message  "更改 %c 标记为(新标记): " old)
		      (read-char))))
     (list old new)))
...
`!' to unmark all remaining files with no more questions."))
	...
	(if (or (not arg)
		(let ((file (dired-get-filename t t)))
		  (and file
		       (dired-query 'query "取消标记文件 %s?"
				    file))))
	    (progn (subst-char-in-region (1- (point)) (point)
					 (preceding-char) ?\s)
		   (setq count (1+ count)))))
      (message (if (= count 1) "1个标记的已移除"
		 "%d个标记的已移除")
	       count))))

...
or nil if file names are not applicable."
  (if (= (length failures) 1)
    ...
    (message (if failures "%s--输入 ? 查看详情 (%s)"
	       "%s--输入 ? 查看详情")
	     string failures))
	  ...
	  (let (case-fold-search)
	    (cond ((string-match
		  ...
		  ((string-match
		   ...
		   "按日期进行 Dired 列队")
		  (t
		   (concat "Dired 列队 " dired-actual-switches)))))
...
With a prefix argument, edit the current listing switches instead."
  ...
  (when dired-sort-inhibit
    (error "无法对这个已进行 dired 列队的缓冲区进行排序"))
  (if arg
      (dired-sort-other
       (read-string "ls 开关(必须包含 -l): " dired-actual-switches))
    (dired-sort-toggle)))
...
Anything else means ask for each directory."
  :type '(choice :tag "复制目录"
		 (const :tag "不进行递归复制" nil)
		 (const :tag "询问每一个目录" t)
		 (const :tag "只询问每一个顶目录" top)
		 (const :tag "复制目录而不进行询问" always))
  :group 'dired)

(defun dired-dnd-popup-notice ()
  (message-box
   "Dired 列队递归复制目前已禁用。\n请查看变量 dired-recursive-copies 。"))
