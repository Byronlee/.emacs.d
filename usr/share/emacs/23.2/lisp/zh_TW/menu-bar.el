...
;;; 2013-05-19 22:09:43 CST translated
...
(define-key global-map [menu-bar help-menu] (cons (purecopy "幫助") menu-bar-help-menu))
...
(define-key menu-bar-file-menu [ps-print-region-faces]
  `(menu-item ,(purecopy "Postscript 列印選區") ps-print-region-with-faces
	      :enable mark-active
	      :help ,(purecopy "Pretty-print marked region to PostScript printer")))
(define-key menu-bar-file-menu [ps-print-buffer-faces]
  `(menu-item ,(purecopy "Postscript 列印緩沖區") ps-print-buffer-with-faces
	      :enable (menu-bar-menu-frame-live-and-visible-p)
...
(define-key menu-bar-search-menu [repeat-search-back]
  `(menu-item ,(purecopy "繼續後退") nonincremental-repeat-search-backward
...
(define-key menu-bar-edit-menu [paste-from-menu]
  `(menu-item ,(purecopy "從剪貼板(即 Kill 菜單)進行粘貼") yank-menu
...
(define-key menu-bar-edit-menu [copy]
  `(menu-item ,(purecopy "復製") menu-bar-kill-ring-save
	      :enable mark-active
...
(defvar menu-bar-line-wrapping-menu (make-sparse-keymap "換行"))

(define-key menu-bar-line-wrapping-menu [word-wrap]
  `(menu-item ,(purecopy "單詞換行(可見行模式) [Word Wrap (Visual Line mode)]")
	      (lambda ()
		(interactive)
...
(define-key menu-bar-options-menu [line-wrapping]
  `(menu-item ,(purecopy "在當前緩沖區換行") ,menu-bar-line-wrapping-menu))
