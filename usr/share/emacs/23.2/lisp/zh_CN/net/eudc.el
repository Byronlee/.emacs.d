...
otherwise they are formatted according to `eudc-user-attribute-names-alist'."
  (let (inhibit-read-only
	...
	first-record
	attribute-name)
    (with-output-to-temp-buffer "*目录查询结果*"
...
server for future sessions."
  (interactive (list
		(read-from-minibuffer "目录服务器: ")
		(intern (completing-read "协议: "
					 (mapcar '(lambda (elt)
  ...
  (if (called-interactively-p 'interactive)
      (message "当前的目录服务器当前 %s (%s)" eudc-server eudc-protocol))
  (if (null no-save)
      (eudc-save-options)))
...
queries the server for the existing fields and displays a corresponding form."
  ...
  (let ((fields (or (and get-fields-from-server
    ...
    (widget-insert "====================\n\n")
    (widget-insert "当前服务器        : " (or eudc-server
					     (progn
					       (call-interactively 'eudc-set-server)
					       eudc-server))
					     "\n")
    (widget-insert "协议             : " (symbol-name eudc-protocol) "\n")
...
(defun eudc-install-menu ()
  (cond
   ...
   ((not (featurep 'xemacs))
	...
	(cons "目录搜索"
	      (easy-menu-create-menu "目录搜索" (cdr (eudc-menu))))))
	...
	[menu-bar tools eudc]
	(cons "目录搜索"
	      (easy-menu-create-keymaps "目录搜索" (cdr (eudc-menu))))))
     (t
      (error "未知版本的 easymenu"))))
   ))
...
(cond
 ((not (featurep 'xemacs))
  (defvar eudc-tools-menu
    (let ((map (make-sparse-keymap "目录搜索")))
      (define-key map [phone]
	`(menu-item ,(purecopy "获取联系电话") eudc-get-phone
		    :help ,(purecopy "Get the phone field of name from the directory server")))
      (define-key map [email]
	`(menu-item ,(purecopy "获取邮件") eudc-get-email
...
;;; eudc.el ends here
;; Maintainer: Pavel Jan韐 <Pavel@Janik.cz>
