...
re-start Emacs."
  :type '(repeat (list (choice :tag "字典"
			       (string :tag "字典名称")
			       (const :tag "default" nil))
		       (regexp :tag "区分字母大小写")
		       (regexp :tag "不区分字母大小写")
...
(if ispell-menu-map-needed
    (progn
      (setq ispell-menu-map (make-sparse-keymap "拼写"))
      (define-key ispell-menu-map [ispell-change-dictionary]
	`(menu-item ,(purecopy "更改字典...") ispell-change-dictionary
		    :help ,(purecopy "Supply explicit dictionary file name")))
      (define-key ispell-menu-map [ispell-kill-ispell]
	`(menu-item ,(purecopy "杀死进程") ispell-kill-ispell
		    :enable (and (boundp 'ispell-process) ispell-process
				 (eq (ispell-process-status) 'run))
		    :help ,(purecopy "Terminate Ispell subprocess")))
      (define-key ispell-menu-map [ispell-pdict-save]
	`(menu-item ,(purecopy "保存字典")
	...
	`(menu-item ,(purecopy "自动拼写检查 (Flyspell)")
		    flyspell-mode
		    :help ,(purecopy "Check spelling while you edit the text")
		    :button (:toggle . (bound-and-true-p flyspell-mode))))
      (define-key ispell-menu-map [ispell-complete-word]
	`(menu-item ,(purecopy "补齐单词") ispell-complete-word
		    :help ,(purecopy "Complete word at cursor using dictionary")))
      (define-key ispell-menu-map [ispell-complete-word-interior-frag]
	`(menu-item ,(purecopy "补齐拆分的单词") ispell-complete-word-interior-frag
...
(if ispell-menu-map-needed
	...
	`(menu-item ,(purecopy "继续拼写检查") ispell-continue
	...
	`(menu-item ,(purecopy "对单词进行拼写检查") ispell-word
		    :help ,(purecopy "Spell-check word at cursor")))
      (define-key ispell-menu-map [ispell-comments-and-strings]
	`(menu-item ,(purecopy "对注释进行拼写检查") ispell-comments-and-strings
...
(if (and (featurep 'xemacs)
    ...
    (let ((dicts (if (fboundp 'ispell-valid-dictionary-list)
	  ...
	  (menu
	   '(["帮助"		(describe-function 'ispell-help) t]
	     ;;["帮助"		(popup-menu ispell-help-list)	t]
	     ["检查信息"	ispell-message			t]
	     ["检查缓冲区"	ispell-buffer			t]
	     ["检查注释"	ispell-comments-and-strings	t]
	     ["检查单词"	ispell-word			t]
	     ["检查区域"	ispell-region  (or (not zmacs-regions) (mark))]
	     ["继续检查"	ispell-continue			t]
	     ["补齐拆分的单词"ispell-complete-word-interior-frag t]
	     ["补齐单词"	ispell-complete-word		t]
	     ["杀死进程"	ispell-kill-ispell		t]
	     ["自定义..."	(customize-group 'ispell)	t]
	     ;; flyspell-mode may not be bound...
	     ;;["flyspell"	flyspell-mode
	     ;;			:style toggle :selected flyspell-mode ]
	     "-"
	     ["保存个人字典"(ispell-pdict-save t t)	t]
	     ["更改字典" ispell-change-dictionary	t])))
      (if (null dicts)
	...
	(setq menu (append menu
      ...
      (if current-menubar
	  (progn
	    (if (car (find-menu-item current-menubar '("命令")))
		(progn
		  ;; XEmacs 21.2
		  (delete-menu-item '("命令" "拼写检查"))
		  (add-menu '("命令") "拼写检查" ispell-menu-xemacs))
	      ;; previous
	      (delete-menu-item '("编辑" "拼写")) ; in case already defined
	      (add-menu '("编辑") "拼写" ispell-menu-xemacs))))))
